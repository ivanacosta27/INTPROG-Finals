...
import com.mab.buwisbuddyph.activities.CreateMessageActivity
    ...
    private fun updateBadgeCounts() {
        ...
        db.collection("Chats")
            ...
            .addOnSuccessListener { chatDocuments ->
                ...
                for (document in chatDocuments) {
                    val isTrash = document.getBoolean("is_trash") ?: false
                    val isReadByUser2 = document.getBoolean("is_read_person_1") ?: false

                    if (!isTrash && !isReadByUser2) {
                        unreadInInbox++
                    } else if (isTrash && !isReadByUser2) {
                        unreadInTrash++
                    }
                }

                // Update badge counts
                inboxBadge = unreadInInbox
                trashBadge =  unreadInTrash
            }
        ...
        db.collection("Chats")
            ...
            .addOnSuccessListener { chatDocuments ->
                ...
                for (document in chatDocuments) {
                    val isTrash = document.getBoolean("is_trash") ?: false
                    val isReadByUser2 = document.getBoolean("is_read_person_2") ?: false
